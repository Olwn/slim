...
if __name__ == '__main__':
  ...
  exp_filter = '2053'
  rates_for_plot = [0.0, 0.25, 0.5, 0.9]
  data_train = {}
  data_test = {}
  for d in os.listdir(exp_dir):
    if not exp_filter in d: continue
    drop_rate = re.search(r"(?<=dp)[0-9\\.]*(?=-)", d).group()

    drop_rate = float(drop_rate)
    if drop_rate not in rates_for_plot: continue
    train_dir = os.path.join(exp_dir, d)
    events_files_train = [x for x in os.listdir(train_dir) if x.startswith('events')]
    data_train[drop_rate] = read_event_data(os.path.join(train_dir, events_files_train[0]), keys_train)

    test_dir = os.path.join(exp_dir, d, 'eval')
    events_files_test = [x for x in os.listdir(test_dir) if x.startswith('events')]
    data_test[drop_rate] = read_event_data(os.path.join(test_dir, events_files_test[0]), keys_test)
  ...
  for (keys, data) in [(keys_train, data_train), (keys_test, data_test)]:
    for tag in keys:
      ...
      key = keys[tag]
      # for i in [0, 1]:
      dp_for_plot = sorted(data_for_key.keys())[0::1]
      plot({k: v for k, v in data_for_key.iteritems() if k in dp_for_plot}, key['title'],
           ...
           save_to='./figures/dp-%s-%s.png' % (tag.replace('/', ''), exp_filter))
