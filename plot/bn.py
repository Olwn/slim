...
if __name__ == '__main__':
  ...
  keys_test = {
    'eval/Accuracy': {'title': 'Evaluation Accuracy', 'y_name': 'Accuracy', 'y_limit': (0.4, 0.9)},
    'eval/ValLoss': {'title': 'Evaluation Cross Entropy Loss', 'y_name': 'Loss', 'y_limit': (1, 2)}
  }
  exp_dir = '/hdd/x/exp/slim'
  exp_filter = '0752'
  rates_for_plot = [0.01, 0.1, 0.5, 1.0]
  data_train = {}
  data_test = {}
  for d in os.listdir(exp_dir):
    if not exp_filter in d: continue
    lr = re.search(r"(?<=lr)[0-9\\.]*(?=-)", d).group()

    lr = float(lr)
    if lr not in rates_for_plot: continue
    train_dir = os.path.join(exp_dir, d)
    events_files_train = [x for x in os.listdir(train_dir) if x.startswith('events')]
    data_train[lr] = read_event_data(os.path.join(train_dir, events_files_train[0]), keys_train)

    test_dir = os.path.join(exp_dir, d, 'eval')
    events_files_test = [x for x in os.listdir(test_dir) if x.startswith('events')]
    data_test[lr] = read_event_data(os.path.join(test_dir, events_files_test[0]), keys_test)
  ...
  for (keys, data) in [(keys_train, data_train), (keys_test, data_test)]:
    for tag in keys:
      ...
      key = keys[tag]
      # for i in [0, 1]:
      lr_for_plot = sorted(data_for_key.keys())[0::1]
      plot({k: v for k, v in data_for_key.iteritems() if k in lr_for_plot}, key['title'],
           x_name=key.get('x_name') or 'step',
           y_name=key.get('y_name') or 'value',
           legend_prefix='lr=',
           x_limit=key.get('x_limit'),
           y_limit=key.get('y_limit'),
           save_to='./figures/bn-%s-%s.png' % (tag.replace('/', ''), exp_filter))
