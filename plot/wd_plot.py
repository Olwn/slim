...
def read_event_data(event_file, keys):
  ...
  for key in data:
    data[key] = sorted(data[key], key=itemgetter(1))
    print data[key]
  return data
...
def plot(data, title, x_name, y_name, legend_prefix, save_to):
  ...
  for key in data:
    ...
    plt.plot(xs, ys, label=legend_prefix + key, ms=2, linestyle='--', linewidth=1)
...
if __name__ == '__main__':
  ...
  for d in os.listdir(exp_dir):
    ...
    weight_decay = re.search(r"(?<=-wd)\S*(?=-lr)", d).group()
    train_dir = os.path.join(exp_dir, d)
    events_files = [x for x in os.listdir(train_dir) if x.startswith('events')]
    data[weight_decay] = read_event_data(os.path.join(train_dir, events_files[0]), keys)
  print(data.keys())
  for key in keys:
    data_for_key = {wd: data[wd][key] for wd in data}
    plot(data_for_key, key, 'steps', 'value', 'wd=', './figures/wd-%s.eps' % key.replace('/', ''))
